...
Add real-time transcription text alongside an Agora RTC session so participants can follow spoken audio as captions.

Use this recipe when you want a working reference for RTC audio, real-time text, transcription UI, token handling.

Repository: [AgoraIO-Community/Agora-RTT-Demo](https://github.com/AgoraIO-Community/Agora-RTT-Demo)
...
```text
You are working in `AgoraIO-Community/Agora-RTT-Demo`. Add or adapt this sample so a developer can run the real-time transcription demo end to end with Agora RTC.
